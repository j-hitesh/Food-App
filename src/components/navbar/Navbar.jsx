...
const menushow = () => {
  ...
  return (
    <div className='flex justify-between w-full h-20 bg-gray-700  text-white px-4 py-4 rounded-b-lg fixed top-0 z-10 mb-10 '>
       {/* <h1 className='bg-pink-500'>check tailwind</h1> */}
      {/* <img  src={assets.logo} alt="logo" className="w-12" /> */}
      <ul className="md:flex gap-x-12  w-1/2 text-md lg:text-2xl sm:text-xl hidden ">
       ...
       <button onClick={menushow} className='block md:hidden'><i className="fa-solid fa-bars fixed top-5"></i></button>
      
      <div>

      <ul className={`top-10 shadow-xl shadow-gray-400 rounded-r-lg relative right-110 px-5 w-70 text-center text-2xl ${showMenuBar ? 'h-30 bg-gray-700' : 'hidden'}`}>
        <li onClick={() => setMenu("Home")} className= {menu === "Home" ? "active" : "font-Tinos hover:text-red-500"}> <Link to='/'>Home</Link></li>
        <li onClick={() => setMenu("Reviews")} className={menu === "Reviews" ? "active" : " font-Tinos hover:text-red-500"}><Link to='/Reviews'>Reviews</Link></li>
        <li onClick={() => setMenu("Cart")} className={menu === "Cart" ? "active" : "font-Tinos hover:text-red-500"}><Link to='/Cart'> Cart</Link></li>
      </ul>
